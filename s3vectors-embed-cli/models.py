...
class SupportedModel(Enum):
    ...
    # Add Nova MME support
    NOVA_MME = ("amazon.nova-2-multimodal-embeddings-v1:0", ModelCapabilities(
        is_async=False,
        supported_modalities=["text", "image"],
        description="Amazon Nova MME",
        supports_multimodal_input=True,
        payload_schema={
            "text": {
                ...
                "singleEmbeddingParams": {
                    ...
                    "embeddingDimension": "{index.dimensions}",
                    "text": {
                        "truncationMode": "END",
                        "value": "{content.text}"
                    }
            ...
            "image": {
                ...
                "singleEmbeddingParams": {
                    ...
                    "image": {
                        "format": "jpeg",
                        "source": {"bytes": "{content.image_base64}"}
                    }
                }
            },
            "multimodal": {
        ...
        response_embedding_path="embeddings[0].embedding"
    ))
    ...
    def _extract_single_path(self, obj: dict, path: str) -> Any:
        ...
        if path.endswith(".*"):
            ...
            if "]." in path:
                # Handle nested access like "embeddings[0].embedding"
                key, rest = path.split("[", 1)
                index_str, nested_path = rest.split("].", 1)
                index = int(index_str)
                result = obj[key][index]
                # Continue with nested key access
